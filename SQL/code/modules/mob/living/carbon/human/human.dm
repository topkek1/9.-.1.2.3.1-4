	...
	if(istype(id))
		if(id.assignment == "Corporate Liaison")
			...
			switch(id.paygrade)
				...
				if(5) . = size ? "HM " : "Hospital Corpsman"
				if(6) . = size ? "SGT " : "Sergeant"
				if(7) . = size ? "TSGT " : "Technical Sergeant"
				if(8) . = size ? "SSGT " : "Staff Sergeant"
				if(9) . = size ? "SFC " : "Sergeant First Class"
				if(10) . = size ? "MSGT " : "Master Sergeant"
				if(11) . = size ? "SMSGT " : "Senior Master Sergeant"
				if(12) . = size ? "1SG " : "First Sergeant"
				if(13) . = size ? "CMSG " : "Chief Master Sergeant"
				if(14) . = size ? "WO " : "Warrant Officer"
				if(15) . = size ? "CWO " : "Chief Warrant Officer"
				if(16) . = size ? "CWO2 " : "Chief Warrant Officer 2"
				if(17) . = size ? "CWO3 " : "Chief Warrant Officer 3"
				if(18) . = size ? "CWO4 " : "Chief Warrant Officer 4"
				if(19) . = size ? "CWO5 " : "Chief Warrant Officer 5"
				if(20) . = size ? "LO " : "Logistics Officer"
				if(21) . = size ? "COMS " : "Communications Officer"
				if(22) . = size ? "2LT " : "Second Lieutenant"
				if(23) . = size ? "1LT " : "First Lieutenant"
				if(24) . = size ? "CPT " : "Captain"
				if(25) . = size ? "MAJ " : "Major"
				if(26) . = size ? "LTCMDR " : "Lieutenant Commander"
				if(27) . = size ? "CMDR " : "Commander"
				if(28) . = size ? "RDML " : "Rear Admiral Lower Half"
				if(29) . = size ? "RADM " : "Rear Admiral Upper Half"
				if(30) . = size ? "VADM " : "Vice Admiral"
				if(31) . = size ? "ADM " : "Admiral"
				if(32) . = size ? "FADM " : "Fleet Admiral"
				if(33) . = size ? "CMDT " : "Commandant"
				if(34) . = size ? "SOPT " : "Sanctum Operative"
				if(35) . = size ? "SFDR " : "Sanctum Founder"
				if(36) . = size ? "SPSGT " : "Supply Sergeant"
				else
					return no_id
	else
		return no_id
